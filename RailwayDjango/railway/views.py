...
def Add_train(request):
    ...
    if request.method == "POST":
        ...
        d = request.POST['dis']
        Add_Train.objects.create(trainname=n,train_no=no,from_city=f,to_city=to,departuretime=de,arrivaltime=a,trevaltime=t,distance=d)
        error=True
    d={"error":error}
...
def user_view_train(request):
    if not request.user.is_authenticated:
        return redirect('login')
    data2=Add_Train.objects.all()
    d2={"data2":data2}
    return render(request, 'user_view_train.html', d2)